        ...
        <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
                ...
                <div className="lg:col-span-2 bg-gradient-to-br from-brand-600 to-indigo-700 p-8 rounded-xl text-white shadow-xl shadow-brand-500/20 relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-4">
                        ...
                        <div className="pt-4 flex gap-4">
                            <button 
                                onClick={onNewAudit}
                                className="px-5 py-2.5 bg-white/10 hover:bg-white/20 rounded-xl text-sm font-bold backdrop-blur-sm transition-all flex items-center gap-2 border border-white/20"
                            >
                                <RefreshCcw strokeWidth={1.5} className="w-4 h-4" />
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                <SectionCard 
                    ...
                    onCopy={(text: string) => copyToClipboard(text, "headline")}
                ...
                <SectionCard 
                    ...
                    onCopy={(text: string) => copyToClipboard(text, "about")}
                ...
                {/* Experience Section */}
                <div className="bg-white rounded-xl border border-border overflow-hidden group hover:border-brand-200 transition-all duration-300">
                    <div className="relative p-8 border-b border-border flex items-center justify-between overflow-hidden">
                        {/* Bento Corner Light */}
                        ...
                        <div className="relative z-10 flex items-center gap-4">
                            <div className="p-3 bg-brand-50 rounded-xl">
                                <Briefcase strokeWidth={1.5} className="w-6 h-6 text-brand-500" />
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-slate-900">{t.results.experience}</h3>
                    ...
                    <div className="p-8 space-y-8">
                        {results.experience.map((exp: ExperienceItem, idx: number) => (
                            <div key={idx} className="space-y-4">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200/60/60 dark:border-slate-800">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {exp.suggestions.map((sug: string, sidx: number) => (
...
const SectionCard: React.FC<SectionCardProps> = ({ 
    ...
    return (
        <div className="relative bg-white rounded-xl border border-border overflow-hidden group hover:border-brand-200 transition-all duration-300">
            {/* Bento Corner Light */}
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-brand-500/5 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
            
            <div className="relative z-10 p-8 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <div className="p-3 bg-slate-50 rounded-xl group-hover:scale-110 transition-transform duration-500">
                        {React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement<{ strokeWidth?: number }>, { strokeWidth: 1.5 }) : icon}
                    </div>
                    <div>
                ...
                {badge && (
                    <div className="hidden sm:block px-3 py-1 bg-brand-50 rounded-full border border-brand-100">
            ...
            <div className="relative z-10 p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    ...
                    <div className="p-5 bg-slate-50 rounded-xl border border-border text-slate-600 text-sm leading-relaxed">
                        {currentText}
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] font-bold text-brand-600 uppercase tracking-widest">{suggestedLabel}</span>
                        </div>
                        {onCopy && (
                            <button 
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-bold transition-all ${
                                    ...
                                    : "bg-slate-100 text-slate-600 hover:bg-brand-600 hover:text-white shadow-sm"
                                }`}
                            >
                                {isCopied ? <Check strokeWidth={1.5} className="w-3 h-3" /> : <Copy strokeWidth={1.5} className="w-3 h-3" />}
                        ...
                        {onRegenerate && (
                            ...
                            >
                                <RefreshCcw strokeWidth={1.5} className={`w-3 h-3 ${isRegenerating ? "animate-spin" : ""}`} />
                                {isRegenerating 
                    ...
                    <div className={`p-5 bg-brand-50/30 rounded-xl border border-brand-100 text-slate-900 font-medium text-sm leading-relaxed ${isTextarea ? "min-h-[150px]" : ""}`}>
                        {suggestedText}
                    </div>
                </div>
            </div>
