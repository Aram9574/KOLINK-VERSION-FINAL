...
const LinkedInPreview: React.FC<LinkedInPreviewProps> = ({ content = '', user, isLoading, language = 'en', onUpdate, onSchedule, viralScore, viralAnalysis }) => {
    ...
    const needsSeeMore = safeContent.length > 210 || safeContent.split('\n').length > 5;
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                                    {t.viralPotential}
                                    {viralScore > 80 && <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full uppercase font-bold tracking-wide">{t.high}</span>}
                                </h4>
                                <p className="text-xs text-slate-500">{t.aiEstimate}</p>
                            </div>
                        </div>
                    ...
                    {showAudit && viralAnalysis && (
                        <div className="px-4 pb-4 border-t border-slate-100 bg-slate-50/50">
                            <div className="grid grid-cols-3 gap-2 mt-4 mb-4">
                                <ScoreBar label={t.hook} score={viralAnalysis.hookScore} />
                                <ScoreBar label={t.readability} score={viralAnalysis.readabilityScore} />
                                <ScoreBar label={t.value} score={viralAnalysis.valueScore} />
                            </div>
                            ...
                            <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 flex gap-3 items-start">
                                ...
                                <div>
                                    <p className="text-[10px] font-bold uppercase text-amber-700 mb-0.5">{t.proTip}</p>
