...
const LoginPage: React.FC = () => {
    ...
    if (isForgotPasswordMode) {
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <Helmet>
                    <title>{language === 'es' ? 'Recuperar Contraseña - Kolink' : 'Reset Password - Kolink'}</title>
                </Helmet>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row">
            <Helmet>
                <title>{isLoginMode ? (language === 'es' ? 'Iniciar Sesión - Kolink' : 'Login - Kolink') : (language === 'es' ? 'Crear Cuenta - Kolink' : 'Sign Up - Kolink')}</title>
            </Helmet>
            ...
            <div className="flex-1 flex flex-col justify-center p-8 lg:p-16 xl:p-24 bg-white relative z-10">
                <div className="max-w-md w-full mx-auto">
                    ...
                    {/* Header */}
                    <div className="mb-10">
                        <Link to="/" className="inline-flex items-center gap-2 font-display font-bold text-2xl text-slate-900 mb-8 hover:opacity-80 transition-opacity">
                        ...
                        <h1 className="text-3xl lg:text-4xl font-bold text-slate-900 mb-3 tracking-tight">
                            {isLoginMode
                                ...
                                : (language === 'es' ? 'Comienza gratis' : 'Get started for free')}
                        </h1>
                        <p className="text-slate-500 text-lg">
                            {isLoginMode
                                ? (language === 'es' ? 'Ingresa a tu estudio viral.' : 'Enter your viral studio.')
                                : (language === 'es' ? 'Únete a los creadores top 1%.' : 'Join the top 1% creators.')}
                        </p>
                    </div>
