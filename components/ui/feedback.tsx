...
const ItsOkayIcon = () => (
  ...
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M14.5 8C14.5 11.5899 11.5899 14.5 8 14.5C4.41015 14.5 1.5 11.5899 1.5 8C1.5 4.41015 4.41015 1.5 8 1.5C11.5899 1.5 14.5 4.41015 14.5 8ZM16 8C16 12.4183 12.4183 16 8 16C3.58172 16 0 12.4183 0 8C0 3.58172 3.58172 0 8 0C12.4183 0 16 3.58172 16 8ZM11.5249 10.8478L11.8727 10.3286L10.8342 9.6329L10.4863 10.1522C9.94904 10.9543 9.0363 11.4802 8.00098 11.4802C6.96759 11.4802 6.05634 10.9563 5.51863 10.1567L5.16986 9.63804L4.13259 10.3356L4.48137 10.8542C5.2414 11.9844 6.53398 12.7302 8.00098 12.7302C9.47073 12.7302 10.7654 11.9816 11.5249 10.8478ZM6.75 6.75C6.75 7.30228 6.30228 7.75 5.75 7.75C5.19772 7.75 4.75 7.30228 4.75 6.75C4.75 6.19772 5.19772 5.75 5.75 5.75C6.30228 5.75 6.75 6.19772 6.75 6.75C4.75 7.30228 5.19772 7.75 5.75 7.75ZM10.25 7.75C10.8023 7.75 11.25 7.30228 11.25 6.75C11.25 6.19772 10.8023 5.75 10.25 5.75C9.69771 5.75 9.25 6.19772 9.25 6.75C9.25 7.30228 9.69771 7.75 10.25 7.75Z"
    />
  </svg>
);
...
const Default = ({ label }: { label: string }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    console.log("Feedback: handleSubmit triggered", { content, selectedEmoji });
    if (!content.trim()) {
      console.warn("Feedback: empty content");
      return;
    }
    
    setIsSubmitting(true);
    try {
      const { success, error } = await feedbackRepository.submitFeedback(
        content,
        selectedEmoji,
        user?.id
      );
      ...
      if (success) {
        ...
        toast.error("Hubo un error al enviar tu feedback. Inténtalo de nuevo.");
      }
    } catch (err) {
  ...
  return (
    <div className="relative">
      ...
      <Material
        type="menu"
        className={clsx(
          "absolute bg-background-100 w-[340px] duration-200 font-sans left-1/2 -translate-x-1/2 z-[100]",
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col gap-2 p-2">
            ...
            <div className="flex items-center gap-1 text-xs text-gray-500 ml-auto">
              <MSupportedIcon /> feedback inteligente
            </div>
          </div>
          <div className="flex justify-between p-3 bg-accents-1 border-t border-accents-2 rounded-b-xl">
            <span className="flex items-center justify-start cursor-pointer">
              <Button
                ...
                shape="rounded"
                className={clsx(
                  "group hover:!bg-blue-300/30 hover:fill-blue-900 !w-8 !h-8 p-0 duration-200",
                  selectedEmoji === 0 ? "!bg-blue-300/50 fill-blue-900" : "bg-transparent fill-gray-900"
                )}
                onClick={(event) => {
              ...
              <Button
                ...
                shape="rounded"
                className={clsx(
                  "hover:!bg-blue-300/30 hover:fill-blue-900 !w-8 !h-8 p-0 duration-200",
                  selectedEmoji === 1 ? "!bg-blue-300/50 fill-blue-900" : "bg-transparent fill-gray-900"
                )}
                onClick={(event) => {
                  event.preventDefault();
              ...
              <Button
                ...
                className={clsx(
                  "hover:!bg-blue-300/30 hover:fill-blue-900 !w-8 !h-8 p-0 duration-200",
                  selectedEmoji === 2 ? "!bg-blue-300/50 fill-blue-900" : "bg-transparent fill-gray-900"
                )}
                onClick={(event) => {
                  event.preventDefault();
              ...
              <Button
                ...
                className={clsx(
                  "hover:!bg-blue-300/30 hover:fill-blue-900 !w-8 !h-8 p-0 duration-200",
                  selectedEmoji === 3 ? "!bg-blue-300/50 fill-blue-900" : "bg-transparent fill-gray-900"
                )}
                onClick={(event) => {
                  event.preventDefault();
            ...
            <Button size="small" type="primary" loading={isSubmitting} htmlType="submit" onClick={handleSubmit}>Enviar</Button>
          </div>
        </form>
      </Material>
    </div>
...
const Inline = ({ label }: { label: string }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    if (e) e.preventDefault();
    console.log("Feedback (Inline): handleSubmit triggered", { content, selectedEmoji });
    if (!content.trim()) {
      console.warn("Feedback (Inline): empty content");
      return;
    }
    ...
    try {
      ...
      if (success) {
        console.log("Feedback (Inline): submission successful");
        toast.success("¡Gracias por tu feedback!");
        setExpanded(false);
        setContent("");
        setSelectedEmoji(null);
      } else {
        console.error("Feedback (Inline): submission failed", error);
        toast.error("Hubo un error al enviar tu feedback. Inténtalo de nuevo.");
      }
    } catch (err) {
      console.error("Feedback (Inline): catch error", err);
      toast.error("Error inesperado. Inténtalo de nuevo.");
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <div
      className={clsx(
        ...
        expanded ? "max-h-[300px] rounded-xl" : "max-h-12 rounded-[30px]"
      )}
      ref={ref}
    >
