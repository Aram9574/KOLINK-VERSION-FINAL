...
import { AppLanguage, ViralAnalysis } from "../../../types";
import {
    Check,
    ChevronDown,
    ChevronUp,
...
interface DemoLinkedInPreviewProps {
    language: AppLanguage;
    isLoading: boolean;
}

const DemoLinkedInPreview: React.FC<DemoLinkedInPreviewProps> = (
    { language, isLoading },
) => {
    // Default high quality content
    const defaultContentEs = `Deja de mirar una página en blanco. 🔴
...
#GrowthHacking #LinkedinTips #AI`;
    ...
    const defaultContentEn = `Stop staring at a blank page. 🔴

Most people lose 10 hours/week thinking about what to write.
...
#GrowthHacking #LinkedinTips #AI`;
    ...
    const [content, setContent] = useState(
        language === "es" ? defaultContentEs : defaultContentEn,
    );
    // Reset content when language changes if user hasn't edited heavily (simple check)
    React.useEffect(() => {
        setContent(language === "es" ? defaultContentEs : defaultContentEn);
    }, [language]);
    ...
    // Mock viral stats
    const viralScore = 92;
    const viralAnalysis: ViralAnalysis = {
        hookScore: 95,
        readabilityScore: 88,
        valueScore: 92,
        feedback: language === "es"
            ? "Excelente uso del framework PAS. El gancho es visual y detiene el scroll. La estructura es escaneable y termina con una conclusión fuerte."
            : "Excellent use of the PAS framework. The hook is visual and stops the scroll. The structure is scannable and ends with a strong takeaway.",
    };
    ...
    if (isLoading) {
        return (
            <div className="bg-white border border-slate-200 rounded-lg shadow-sm w-full mx-auto p-4 animate-pulse h-[400px]">
...
const ActionButton: React.FC<{ icon: React.ReactNode; label: string }> = (
    ...
    <button className="flex items-center justify-center gap-2 px-2 sm:px-4 py-3.5 rounded-lg hover:bg-slate-100 flex-1 text-slate-500 font-semibold text-sm transition-all duration-200 group active:scale-95 active:bg-slate-200">
        <span className="group-hover:scale-110 transition-transform duration-200 group-hover:text-slate-700">
            {icon}
        </span>
...
export default DemoLinkedInPreview;
