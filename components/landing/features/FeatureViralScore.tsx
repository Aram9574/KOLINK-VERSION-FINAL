...
const FeatureViralScore: React.FC<FeatureViralScoreProps> = ({ language }) => {
    ...
    return (
        <div className="col-span-12 lg:col-span-4 bg-white border border-slate-200 rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden group shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
            <div className="absolute -right-20 -top-20 w-64 h-64 bg-green-50 rounded-full blur-3xl group-hover:bg-green-100 transition-colors duration-500"></div>
            ...
            <div className="relative z-10 h-full flex flex-col">
                ...
                <div className="mt-auto bg-slate-50 rounded-2xl p-5 border border-slate-100 group-hover:border-green-100 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t.features.viralScore}</span>
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                            <span className="text-2xl font-bold text-slate-900">94<span className="text-slate-400 text-sm">/100</span></span>
                        </div>
                    </div>
                    {/* Progress Bar with Animation */}
                    <div className="h-4 w-full bg-slate-200 rounded-full overflow-hidden mb-6 relative">
                        <div className="absolute inset-0 bg-slate-200 w-full"></div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-white rounded-xl p-3 text-center shadow-sm border border-slate-100 group-hover:border-green-100 transition-colors">
                            <span className="block text-[10px] text-slate-400 font-bold uppercase mb-1">{t.features.hooks}</span>
                            <span className="inline-block px-2 py-0.5 rounded-md bg-green-100 text-green-700 font-bold text-sm">A+</span>
                        </div>
                        <div className="bg-white rounded-xl p-3 text-center shadow-sm border border-slate-100 group-hover:border-green-100 transition-colors">
                            <span className="block text-[10px] text-slate-400 font-bold uppercase mb-1">{t.features.format}</span>
                            <span className="inline-block px-2 py-0.5 rounded-md bg-green-100 text-green-700 font-bold text-sm">A</span>
                        </div>
                    </div>
                </div>
