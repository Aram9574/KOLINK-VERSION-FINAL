...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 4. Get/Create Customer logic (UPDATED for subscriptions table)
    // Check subscriptions table first for existing connection
    ...
    if (!customerId) {
        ...
        // Upsert into subscriptions table to link this customer ID
        // This creates an initial 'free/inactive' subscription record if none exists
        const { error: upsertError } = await supabaseAdmin
            .from("subscriptions")
            .upsert({ 
                user_id: user.id,
                stripe_customer_id: customerId,
                status: 'incomplete_expired', // Default state until checkout succeeds
                plan_type: 'inicial'
            }, { onConflict: 'user_id' }); // Assuming one sub per user for now, or match logic

        if (upsertError) {
            console.error("Failed to upsert subscription with customer ID:", upsertError);
            // Continue properly even if DB sync fails, we have customerId for Stripe
        }
    }
    ...
    try {
      ...
      session = await stripe.checkout.sessions.create({
       ...
       // Error handling remains similar, can be improved to update subscriptions table on recovery
       throw error;
    }
